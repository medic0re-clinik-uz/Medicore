...
function applyTheme(t) {
  ...
  if (label) label.textContent = t === 'dark' ? 'Kunduzgi rejim' : 'Tungi rejim';
}
...
function staCls(s) {
  return {"Davolanmoqda":"badge-primary","Kuzatuvda":"badge-warning","Sog'aydi":"badge-success","Faol":"badge-success","Nofaol":"badge-gray"}[s]||'badge-gray';
}
...
async function doLogin() {
  ...
  if (!username || !password) {
    errEl.textContent = '⚠️ Login va parolni kiriting';
...
const MENUS = {
  admin: [
    ...
    { key:'doctors',    label:'Shifokorlar',        icon:'⚕' },
    { key:'patients',   label:'Bemorlar',           icon:'♥' },
    { key:'diseases',   label:'Kasalliklar',        icon:'⊕' },
    { key:'clinic',     label:'Klinik ma\'lumot',   icon:'⊡' },
    { key:'emergency',  label:'Favqulodda aloqa',   icon:'!' },
  ],
  clinician: [
    { key:'dashboard',  label:'Bosh sahifa',        icon:'⊞' },
    { key:'patients',   label:'Bemorlar',           icon:'♥' },
  ...
  receptionist: [
    { key:'dashboard',     label:'Bosh sahifa',     icon:'⊞' },
    { key:'patients',      label:'Bemorlar',        icon:'♥' },
    { key:'doctors_view',  label:'Shifokor jadvali',icon:'⚕' },
    { key:'emergency',     label:'Favqulodda aloqa',icon:'!' },
  ],
};
...
async function renderDashboard() {
  ...
  el.innerHTML = `<div class="page-header"><h1>Bosh sahifa</h1><p>CareTrack Clinic — Tibbiy Yozuvlarni Boshqarish Tizimi</p></div>
    <p class="text2">Yuklanmoqda...</p>`;
  try {
    const [stats, doctors, patients] = await Promise.all([Stats.get(), Doctors.getAll(), Patients.getAll()]);
    const isAdmin = currentUser.role === 'admin';
    const cards = [
      isAdmin ? `<div class="card stat-card"><div class="stat-icon" style="background:var(--primary-light)">⚕</div><div><div class="stat-label">Shifokorlar</div><div class="stat-value">${stats.doctors}</div></div></div>` : '',
      `<div class="card stat-card"><div class="stat-icon" style="background:var(--accent-light)">♥</div><div><div class="stat-label">Bemorlar</div><div class="stat-value">${stats.patients}</div></div></div>`,
      `<div class="card stat-card"><div class="stat-icon" style="background:var(--warning-light)">⊕</div><div><div class="stat-label">Kasalliklar</div><div class="stat-value">${stats.diseases}</div></div></div>`,
      `<div class="card stat-card"><div class="stat-icon" style="background:var(--success-light)">✓</div><div><div class="stat-label">Sog'aydilar</div><div class="stat-value">${stats.recovered}</div></div></div>`,
      `<div class="card stat-card"><div class="stat-icon" style="background:var(--danger-light)">⊗</div><div><div class="stat-label">Davolanmoqda</div><div class="stat-value">${stats.treating}</div></div></div>`,
    ].join('');
    ...
    el.innerHTML = `
      <div class="page-header"><h1>Bosh sahifa</h1><p>CareTrack Clinic — Tibbiy Yozuvlarni Boshqarish Tizimi</p></div>
      <div class="stats-grid">${cards}</div>
      <div class="dash-grid">
        <div class="card dash-card"><h3>So'nggi bemorlar</h3>${recPats||'<p class="empty">Ma\'lumot yo\'q</p>'}</div>
        <div class="card dash-card"><h3>So'nggi kasalliklar</h3>${recDis||'<p class="empty">Ma\'lumot yo\'q</p>'}</div>
      </div>`;
  } catch(err) {
...
async function renderDoctors(q='') {
  ...
  const isAdmin = currentUser.role === 'admin';
  el.innerHTML = `
    <div class="page-header"><h1>Shifokorlar</h1><p>Klinikadagi barcha shifokorlar boshqaruvi</p></div>
    <div class="toolbar">
      <input class="search-input" id="doc-q" placeholder="Ism, mutaxassislik, bo'lim bo'yicha qidirish..." value="${escHtml(q)}"
        oninput="debounced(()=>renderDoctors(document.getElementById('doc-q').value))"/>
      ${isAdmin?`<button class="btn btn-primary" onclick="openDoctorForm()">+ Yangi shifokor</button>`:''}
    </div>
    <div class="table-wrap"><p style="padding:20px" class="text2">Yuklanmoqda...</p></div>`;
  try {
    const [docs, patients] = await Promise.all([Doctors.getAll(q), Patients.getAll()]);
    const heads = ['Shifokor','Mutaxassislik',"Bo'lim",'Tajriba','Telefon','Holat','Bemorlar',...(isAdmin?['Amallar']:[])];
...
function openDoctorForm(doc=null) {
  ...
  openModal(doc?'Shifokorni tahrirlash':"Yangi shifokor qo'shish", `
    <div class="form-grid">
      ...
      <div><label class="fg-label">Holat</label>
        <select class="fg-select" id="f-status">
          <option ${val('status')==='Faol'?'selected':''}>Faol</option>
          <option ${val('status')==='Nofaol'?'selected':''}>Nofaol</option>
        </select></div>
    </div>
    <div class="form-actions">
      <button class="btn btn-outline" onclick="closeModal()">Bekor qilish</button>
      <button class="btn btn-primary" onclick="saveDoctor(${doc?doc.id:'null'})">Saqlash</button>
    </div>`, true);
}
...
async function renderPatients(q, doctorId) {
  ...
  el.innerHTML = `
    <div class="page-header"><h1>Bemorlar</h1><p>Ro'yxatdan o'tgan barcha bemorlar</p></div>
    <div class="toolbar">
  ...
  try {
    ...
    el.querySelector('.table-wrap').innerHTML = `<table>
      <thead><tr>${['Bemor','Yosh / Jins','Telefon','Qon guruhi','Shifokor','Kasalliklar','Amallar'].map(h=>`<th>${h}</th>`).join('')}</tr></thead>
...
async function openPatientForm(pat=null) {
  ...
  openModal(pat?"Bemorni tahrirlash":"Yangi bemor ro'yxatga olish", `
    ...
    <div class="form-actions">
      <button class="btn btn-outline" onclick="closeModal()">Bekor qilish</button>
      <button class="btn btn-primary" onclick="savePatient(${pat?pat.id:'null'})">Saqlash</button>
    </div>`, true);
}
    ...
    openModal('Bemor profili', `
      <div class="profile-grid">
        <div class="profile-info">
          ...
          <p>📞 ${escHtml(p.phone||'—')}<br>📍 ${escHtml(p.address||'—')}<br>📅 Ro'yxat: ${p.registeredAt||'—'}</p>
        </div>
        <div class="profile-info">
...
async function renderDiseases(filters) {
  ...
  const isAdmin = currentUser.role==='admin';
  el.innerHTML = `
    <div class="page-header"><h1>Kasalliklar / Tashxislar</h1><p>Barcha bemor kasallik va tashxis yozuvlari</p></div>
    <div class="toolbar">
      ...
      <select class="filter-select" id="dis-sta" onchange="getDisFilter()">
        ...
        ${["Davolanmoqda","Kuzatuvda","Sog'aydi"].map(s=>`<option ${_disFilters.status===s?'selected':''}>${s}</option>`).join('')}
      </select>
  ...
  try {
    ...
    const heads = ['ICD Kodi','Kasallik','Bemor','Sana',"Og'irlik darajasi",'Holat','Amallar'];
...
async function openDiseaseForm(dis=null) {
  ...
  openModal(dis?'Kasallikni tahrirlash':"Yangi kasallik yozuvi", `
    <div class="form-grid">
      ...
      <div><label class="fg-label">Holat</label>
        <select class="fg-select" id="fd-sta">
          ${["Davolanmoqda","Kuzatuvda","Sog'aydi"].map(s=>`<option ${val('status')===s?'selected':''}>${s}</option>`).join('')}
        </select></div>
    ...
    <div class="form-actions">
      <button class="btn btn-outline" onclick="closeModal()">Bekor qilish</button>
      <button class="btn btn-primary" onclick="saveDisease(${dis?dis.id:'null'})">Saqlash</button>
    </div>`, true);
}
...
async function renderClinic() {
  ...
  try {
    ...
    const services = [
      ...
      "Bo'limlar bo'ylab bemor tarixini kuzatish",
      "Bo'limlar o'rtasida yo'llanma boshqarish",
    ];
    ...
    el.innerHTML = `
      ...
      <div class="info-section">
        ...
        <div class="info-grid">
          ...
          <div class="info-item"><div class="info-item-label">Manzil</div><div class="info-item-value">Toshkent sh., Yunusobod tumani</div></div>
          <div class="info-item"><div class="info-item-label">Asosiy telefon</div><div class="info-item-value">+998 71 200-10-00</div></div>
          <div class="info-item"><div class="info-item-label">Email</div><div class="info-item-value">info@caretrack.uz</div></div>
          <div class="info-item"><div class="info-item-label">Ish vaqti</div><div class="info-item-value">Dush–Shanba, 08:00–18:00</div></div>
      ...
      <div class="info-section">
        ...
        <div class="info-grid">
          ...
          <div class="info-item"><div class="info-item-label">Davolanmoqda</div><div class="info-item-value" style="color:var(--primary);font-size:18px;font-weight:700">${diseases.filter(d=>d.status==="Davolanmoqda").length}</div></div>
...
function renderEmergency() {
  const el = document.getElementById('page-content');
  const contacts = [
    { dept:"Kardiologiya — Favqulodda",   phone:"+998 71 200-11-99", avail:"24/7", doc:"Dr. Alisher Karimov",    note:"Yurak xurujlari, o'tkir koronar sindrom" },
    { dept:"Nevrologiya — Favqulodda",    phone:"+998 71 200-12-99", avail:"24/7", doc:"Dr. Nilufar Rashidova",  note:"Insult, miyaga qon quyilish holatlari" },
    { dept:"Ortopediya — Favqulodda",     phone:"+998 71 200-13-99", avail:"24/7", doc:"Dr. Malika Yusupova",   note:"Suyak sinishi, travmatologiya" },
  ...
  el.innerHTML = `
    <div class="page-header"><h1>Favqulodda aloqa</h1><p>Ish vaqtidan tashqari va favqulodda tibbiy yordam xizmati</p></div>

    <div class="emergency-banner">
      <div class="emergency-icon">🚨</div>
      <div>
        <div class="emergency-title">Favqulodda vaziyatda — 24/7 xizmat</div>
    ...
    <div class="info-section">
      <h2>📞 Bo'limlar favqulodda kontaktlari</h2>
      <div class="table-wrap" style="box-shadow:none;border-radius:8px">
        <table style="min-width:600px"><thead><tr>
          <th>Bo'lim</th><th>Telefon</th><th>Mavjudlik</th><th>Mas'ul shifokor</th><th>Izoh</th>
        </tr></thead><tbody>${rows}</tbody></table>
      </div>
    </div>

    <div class="info-section">
      <h2>📋 Favqulodda holatlarda harakatlar tartibi</h2>
      <ul class="service-list">
        ${steps.map((s,i)=>`<li>
    ...
    <div class="info-section">
      ...
      <p class="text2" style="font-size:13.5px;line-height:1.8">
        Favqulodda tibbiy yordam kerak bo'lganda avval <strong style="color:var(--text)">103</strong> (Tez yordam)ga murojaat qiling.
